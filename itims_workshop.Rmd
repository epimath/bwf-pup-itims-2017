...
Please make sure you downloaded the following software and R pacakges.
...
The files we will need for this exercise are listed below:
...
empty_example_model.nlogo | the empty example model used in **Probelm 1C**
...
*	Consider the parameters for `virus-spread-chance`, `recovery-chance`, and `gain-resistance-chance`. How would you expect these parameters to change the outcome of the disease outbreak? Are there any thresholds you might expect to see, where the outbreak dynamics change once you are above or below a threshold?

* Adjust the three parameters in the previous question to a range of values—how does the model behavior change? 
...
Take a look at the following models with a little bit more complexity. Play with the models yourself, but feel free to ask questions and disscuss with your group!

* **Exploring rotation size parameter from problem 1C**
    + Open [turtle_color.nlogo](turtle_color.nlogo) in NetLogo
    + Similar to the model from problem 1C, but here we start with all turtles in the middle, and add a slider (turn-size) to change the size of rotation for the turtles at each time point (see **Code** for more details). 
    + Try different values of rotation size, how does the pattern change when you run the model?
<br /><br />

* **Exploring how to structure models** - Population versus agent based models (ABM) and deterministic versus stochastic models
    + Author: [Maria Riolo](https://lsa.umich.edu/cscs/people/post-docs-lecturers-visiting-scholars/maria-riolo.html)
    + Population model: [SIR-deterministic.nlogo](SIR-deterministic.nlogo), [SIR-stochastic.nlogo](SIR-stochastic.nlogo)
    + ABM: [SIR-agent-based.nlogo](SIR-agent-based.nlogo)
    ...
    + What is the transmission process of SIR-agent-based model, and how does it differ from SIR-stochastic model?
<br /><br />
...
* **Parameter Estimation Tutorial**
    + Download the files of **param-estimation-SIR** from [epimath](https://github.com/epimath/param-estimation-SIR) repositories by clicking  <img src="figure/icon3.png" alt="Smiley face" height="70" width="90">
    + This example shows how to estimate parameters of interest 
    + It also includes code in MATLAB and Python
<br /><br />
...
* **Deterministic Repressilator Model** from A synthetic oscillatory network of transcriptional regulators, by Elowitz and Leibler.
    + More details can be found [here](Elowitz_Leibler.pdf)
    + Given the description, how would you make it into a stochastic model?
    + Try adjusting the parameters---how does the model behavior change?
